...
const mongo_controller_1 = __importDefault(require("@mhazaa/mongo-controller"));
class AnalyticsEngine {
    static connected = false;
    static collection;
    static connectUsingCollection(collection) {
        AnalyticsEngine.connected = true;
        AnalyticsEngine.collection = collection;
        console.log('Successfully connected to AnalyticsEngine');
    }
    static async connectUsingDBConfig(config) {
        const db = new mongo_controller_1.default(config);
        await db.connect();
